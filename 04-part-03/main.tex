...
% Inductive biases: factors that lead a learner to favor one hypothesis over another that are independent of the observed data. When two hypotheses fit the data equally well, inductive biases are the only basis for deciding between them. Inductive biases are great ways for encoding modeling assumptions. 
...
A well-known example is the translation invariant assumption in the convolutional neural networks~\citep{}\todo{cite Backpropagation Applied to Handwritten Zip Code Recognition} for vision tasks based on a certain symmetry in the data, which considers that a given feature, of any complexity, can appear anywhere in the image.   Other examples include neural networks that encode rotational invariance \todo{cite steerable CNNs} or permutation invariance \todo{cite Deep Sets, Set Transformer, Deep Models of Interactions Across Sets}.
