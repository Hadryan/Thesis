...
% Here is a list of main contributions of this thesis to in solving problems in machine learning, natural language processing, and information retrieval:
% \begin{itemize}
...
%     \item We presented a new family of models, significant words language models, that capture only and all essential features for representing a set of documents and showed not only they are easily inspectable by human, but also effective in classification, (pseudo)-relevance feedback for document ranking, and contextual suggestion tasks. 
%     \item We proposed using heuristic unsupervised models as weak labelers to create a large-scale weakly annotated training set, for tasks where no such training set is available like document ranking, and studied the effectiveness of different general architectures for neural rankers and various objective functions when learning with weak supervision. Our work provided a groundwork for many research on 
%     \item We proposed an approach for learning to learn from weak supervision, by jointly learning the main task's objective as well as labels' quality.
%     \item We proposed a modified version 
% \end{itemize}
